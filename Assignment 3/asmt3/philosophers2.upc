 ...
 * upc_lock() instead of upc_lock_attempt(). To avoid deadlock, even-numbered 
 * philosophers pick up their left forks first and then their right, while odd-
 * numbered philosophers pick up their right forks first and then their left.
 *
 */
...
void life_cycle(void)
{
  ...
  { 
    ...
    /* odd-numbered philosophers pick up their left forks first and then their 
       right */
    if (MYTHREAD % 2 == 1) {
	upc_lock(a_fork[left]);
	upc_lock(a_fork[right]);
    }
    /* even-numbered philosophers pick up their right forks first and then their 
       left */
    else {
	upc_lock(a_fork[right]);
	upc_lock(a_fork[left]);
